...
void testAndColor(float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3, float z3){
    ...
    maxX = (int)((max_x-startImageX)/imageDis);
    maxY = (int)((max_y-startImageY)/imageDis);
    minX = (int)((min_x-startImageX)/imageDis);
    minY = (int)((min_y-startImageY)/imageDis);
    //cout<<zBuffer[1][1]<<endl;
    for (int i = minX;i<=maxX;i++){
        for (int j = minY;j<=maxY;j++){
            //cout<<zBuffer[i][j]<<endl;
            if(zBuffer[i][j]==0)
                zBuffer[i][j] = zValue;
            if(isIn(x1,y1,x2,y2,x3,y3,startImageX+(i+1)*imageDis, startImageY+(j+1)*imageDis) && zValue<=7){
                zBuffer[i][j] = zValue;
                cout<<i<<" "<<j<<" "<<zBuffer[i][j]<<endl;
                glBegin(GL_POINTS);
